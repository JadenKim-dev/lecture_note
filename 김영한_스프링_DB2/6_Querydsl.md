...
만약 SQL이 클래스처럼 자바 코드로 작성 가능해서 타입 체킹이 가능하다면 어떨까?  
이 경우 SQL 자체가 type-safe 해져서 컴파일 시 에러 체킹이 가능해진다.  
또한 IntelliJ 등의 IDE를 사용한다면 코드 어시스트의 도움을 받을 수도 있다.

Querydsl은 쿼리를 type-safe 하게 작성할 수 있게 지원하는 라이브러리이다.  
...
JPQL을 직접 작성하기 위해서는 다음과 같이 쿼리를 구성해야 한다.
...
JPQL은 sql과 유사해서 쿼리를 편리하게 작성할 수 있지만, 문자열로 구성하기 때문에 타입 체킹이 불가능하다.

Criteria API는 쿼리를 type safe하게 구성하기 위해 JPA에서 공식으로 지원하는 라이브러리이다.
...
다만 사용하기 매우 복잡하고, 엔티티의 각 프로퍼티들은 type safe하게 지원되지 않기 때문에 잘 사용되지 않는다.
...
프로젝트 구성 시 APT를 사용하기 위한 설정만 해두면, 손쉽게 라이브러리를 사용할 수 있다.

QueryDSL은 JPQL의 기본 기능을 모두 지원하고, 추가로 동적 쿼리를 편리하게 작성하는 기능을 제공한다.
...
스프링 데이터 JPA는 조회와 관련해서 지원하는 기능이 부족한 편이다.  
QueryDSL은 복잡한 쿼리와 동적 쿼리를 편리하게 구성할 수 있게 지원하여 스프링 데이터 JPA의 부족한 점을 보완해준다. 
...
```
Querydsl을 사용하기 위해서는 EntityManager를 주입받고, 이를 이용해서 JPAQueryFactory 객체를 생성해야 한다.
...
이를 좀 더 최적화하면 itemName, price에 대한 BooleanExpression을 반환하는 메서드를 각각 정의하는 식으로 구성할 수 있다.  
조건을 추가할 때에는 해당하는 함수형 값을, 추가하지 않을 때에는 null을 반환하도록 구현하면 된다.  
그리고 해당 메서드의 반환값을 query.where에 그대로 사용하면 된다.
