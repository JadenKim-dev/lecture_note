...
최신 스프링 버전에서 h2 database를 mysql 모드로 사용하기 위해서는, datasource.url을 명시하고 MODE를 명시해줘야 한다.  
따라서 application.yml에 다음과 같이 설정해야 한다.
...
> `DB_CLOSE_DELAY=-1`은 db 커넥션이 모두 종료되어도 memory db가 종료되지 않게 하기 위한 옵션이다.
