...
`select * from Member`    : JPQL을 그대로 SQL로 변역(1)  
`select * from Team where TEAM_ID = xxx`  : 각 member에 대해서 team을 조회(N)
...
만약 다수의 테이블이 즉시 로딩으로 연결되어 있으면, 쿼리가 여러 개 발생해서 시스템이 매우 느려질 수 있다.  
따라서 가능한 지연로딩을 사용하여 불필요한 쿼리가 발생하지 않게 해야 한다.  
