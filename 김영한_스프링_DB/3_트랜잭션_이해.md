...
만약 조회를 할 때 lock을 획득하고 싶다면 select for update를 사용할 수 있다. 보통 조회한 데이터를 기반으로 시간이 오래 걸리는 복잡한 연산을 수행해야 할 때 사용한다. 연산을 수행하는 중간에 데이터가 변경되는 것을 막기 위함이다.
...
ㅎㅐ당 로직을 다음의 테스트 코드를 통해 실행해 볼 수 있다.  
...
기존에는 레포지토리 안의 각 메서드가 datasource를 통해 각자 커넥션을 생성해서 db 작업을 한 뒤 커넥션을 닫는 식으로 동작했다.
...
따라서 트랜잭션 활용하기 위해서는 커넥션 객체를 파라미터로 받는 식으로 구성해야 한다.  
...
이 때 레포지토리의 각 메서드는 파라미터로 커넥션을 받아서, 이를 이용해서 db에 임시 작업을 수행한다.
