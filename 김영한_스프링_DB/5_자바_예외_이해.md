...
(생성자에서 Throwable 타입으로 예외를 받도록 처리).  
이렇게 하면 본래의 예외와 감싼 예외에 대한 정보를 모두 로그에서 확인할 수 있다.  
이를 통해 이제 해당 예외들을 처리할 수 없는 서비스, 컨트롤러 계층에서는 더이상 던지는 예외를 명시하지 않아도 된다.

자바는 초기 설계단계에서 체크 예외를 기본으로 사용했으나, 점점 라이브러리가 늘어나면서 모든 예외를 명시하기가 어려워졌다.  
요즘엔 대부분의 라이브러리들이 체크 예외를 런타임 예외로 감싸서 제공한다.
