...
웹 만약 모든 체크 예외를 그대로 처리해야 한다고 해보자.  
웹 서버에서는 기본적으로 DB 작업에 문제가 생긴 SQLException이나, 네트워크 통신에 문제가 생긴 ConnectException이 발생할 수 있다.  
이들은 상위 계층인 서비스, 컨트롤러에서 잡아서 복구하는 것이 불가능하다.  
따라서 보통 예외를 계속 상위로 던져서 최상단의 Filter, ControllerAdvice 등에서 적절한 에러메시지를 반환하고 개발자가에게 알람을 보내는 식으로 처리 된다.  
이 때 체크 예외를 사용하면 이를 처리할 수 없는 서비스, 컨트롤러 등 상위 계층에서도 해당 예외를 명시해야 한다. 
