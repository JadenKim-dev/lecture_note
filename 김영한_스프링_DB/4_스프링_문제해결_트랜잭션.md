...
트랜잭션을 시작하고, 커밋/롤백하는 코드가 서비스 로직에 들어가 있는 문제가 있다.  
트랜잭션을 사용하는 부분에서는 해당 코드가 중복해서 들어가고, 달라지는 것은 비즈니스 로직 부분 뿐이다.

스프링에서는 이런 문제를 해결하기 위해 template callback 패턴을 이용한다.  
TransactionTemplate에서는 비즈니스 로직을 담고 있는 콜백 메서드(람다)를 파라미터로 받아서, 이를 사용해 트랜잭션으로 감싸서 실행시켜준다.  
txTemplate.execute((status) -> ...) 로 실행하면 트랜잭션으로 해당 로직을 실행한다.

이를 통해 서비스 계층에서 트랜잭션 관련 처리 코드를 제거할 수 있었다.  
다만 TransactionTemplate 관련 코드는 여전히 남아 있다.  
서비스 로직을 순수하게 유지하기 위해서는 해당 부분도 제거가 필요하다.
