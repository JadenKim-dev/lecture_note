...
서비스 로직에서 트랜잭션 매니저에 트랜잭션 시작을 요청하면 DataSource를 통해 커넥션을 생성하고, autocommit false로 설정한 뒤, 동기화 매니저에 커넥션을 저장한다.  
레포지토리의 메서드에서는 DataSourceUtils를 통해 커넥션을 얻어오고 릴리즈하기 때문에, 서비스 로직이 실행되는 동안 동기화된 커넥션이 유지된다.

마지막에 서비스 로직에서 트랜잭션 매니저에 commit, rollback을 요청하면 db에 커밋 롤백 실행을 요청한 뒤, autocommit을 false로 설정하고 커넥션을 닫게 된다.
(커넥션풀 사용 중이면 풀에 반납됨)
위 방법을 통해 이제 레포지토리 메서드에서 커넥션을 파라미터로 받지 않아도 되도록 개선되었다.
