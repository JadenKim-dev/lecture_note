...
이 때 getConnection을 호출하면 트랜잭션 동기화 매니저에 의해 동기화되어 있는 커넥션이 있는 경우 이를 얻어오고, 없는 경우 새롭게 커넥션을 생성한다.  
releaseConnection을 호출한 경우에는 동기화된 커넥션의 경우 그대로 유지하고, 그렇지 않은 경우 커넥션을 닫는다.  
이를 통해 동일한 인터페이스로 트랜잭션을 사용하는 경우와 autocommit 모드로 사용하는 경우를 모두 대응할 수 있기 때문에, 더이상 레포지토리 메서드에서 파라미터로 커넥션을 받지 않아도 된다.
