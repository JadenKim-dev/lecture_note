...
@Transactional 을 통해 생성된 AOP 객체는 지금까지 해온 플로우를 내부적으로 실행한다.  
스프링 컨테이너에 등록된 TransactionManager를 주입 받아서 getTransaction()을 통해 트랜잭션을 시작한다.  
커넥션이 시작되고, autocommit false로 설정된 커넥션이 트랜잭션 동기화 매니저에 저장된다.  
프록시 서비스 객체에서 실제 비즈니스 로직을 호출하면 레포지토리의 메서드가 호출이 되고, DatasourceUtils를 통해 커넥션을 꺼내오기 때문에 트랜잭션 동기화 매니저에 저장된 트랜잭션을 가져와서 데이터 처리가 이루어진다.

지금까지 해온 것은 코드로 하나하나 다 구현하는 프로그래밍 방식의 사용에서, 선언 하나로 적용을 끝내는 선언적 방식으로 전환해 온 것이라고 할 수 있다.  
Transactional 사용을 위해서는 스프링 컨테이너를 사용해야 하는 제약 사항이 있으나, 대부분의 사비스가 스프링을 사용하기 때문에 실무에서도 거의 선언적 방식의 트랜잭션 사용을 채택한다.
