...
또한 On-Premise 솔루션을 이용해서 할당받는 방법도 있는데, storag e os, ceph, GlusterFS 등을 사용할 수 있다.  
이 솔루션들을 사용하면 알아서 노드 자원을 이용해서 볼륨을 관리해준다.  
이 밖에도 NFS를 이용해서 다른 서버를 볼륨 자원으로 사용하는 것도 가능하다.
...
또한 PV에는 PVC와 연결되어 있는지, 에러가 남진 않았는지 등의 상태값이 존재하고, PV를 삭제하는 정책 등을 지정하는 것도 가능하다.  
